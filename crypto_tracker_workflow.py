...
def main() -> None:
    ...
    logger.info(
        f"Starting crypto workflow with following datetime stamp: {analysis_timestamp}"
    )
    ...
    ListingsStep(analysis_timestamp).generate_listings()
    ...
    UniverseStep(analysis_timestamp).generate_universe()
    ...
    PricingStep(analysis_timestamp).generate_pricing()
    ...
    BitcoinComparisonStep(analysis_timestamp).generate_bitcoin_comparison()
    ...
    df: pd.DataFrame = AverageDifferenceStep(
        analysis_timestamp
    ).generate_average_difference()
